...
// src/app/(main)/dashboard/district/[districtId]/report-case/page.tsx
'use client';
...
export default function DistrictCaseReportForm() {
  ...
  useEffect(() => {
    const loadFacilities = async () => {
      if (status === 'authenticated' && session && districtId) {
        try {
          if (session.user?.role === USER_ROLES.DISTRICT_OFFICER && session.user.district === districtId) {
  ...
  const {
    ...
    defaultValues: {
      ...
      district: districtId, // Pre-fill with the district from the URL
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Report suspected or confirmed cases of epidemic-prone diseases in {districtId}
          </p>
        </div>
            ...
            <FormFieldset legend="Location Information">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Select
                  ...
                  {...register('district')}
                  value={districtId} // Pre-filled with district from URL
                  disabled // Disabled since it's based on the URL
                />
