...
export default function FacilityCaseReportForm() {
  ...
  const {
    ...
    defaultValues: {
      reporterFacility: facilityId, // Pre-fill with the facility from the URL
      reporterRole: session.user?.role || '',
      district: facilityInfo?.district || session.user?.district || '', // Pre-fill with facility's district
  ...
  const onSubmit = async (data: CaseReportFormData) => {
    ...
    try {
      ...
      const submissionData = {
        ...
        reporterName: session.user?.name,
        reporterEmail: session.user?.email,
        reporterFacility: facilityInfo?.name || data.reporterFacility,
      };
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Report suspected or confirmed cases of epidemic-prone diseases from {facilityInfo?.name || 'your facility'}
          </p>
        </div>
