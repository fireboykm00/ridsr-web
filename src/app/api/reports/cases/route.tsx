...
import {
  reportService,
  ReportFilters,
} from "@/lib/services/server/reportService";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const filters: ReportFilters = {
      facilityId,
      district:
        typeof validated.district === "string" ? validated.district : undefined,
      diseaseCode: validated.diseaseCode as DiseaseCode | undefined,
      status:
        typeof validated.status === "string"
          ? (validated.status as CaseStatus)
          : undefined,
      validationStatus:
        typeof validated.validationStatus === "string"
          ? (validated.validationStatus as ValidationStatus)
          : undefined,
    };
    ...
    const uniqueFacilities = [
      ...new Set(
        reportData.cases
          .map((c: { facilityName: string; facilityCode?: string }) =>
            c.facilityCode
    ...
    const districtValue =
      typeof validated.district === "string"
        ? validated.district
        : validated.district?.[0];
    const districtDisplay = districtValue
